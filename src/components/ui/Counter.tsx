...
function Digit({ place, value, height, digitStyle }: DigitProps) {
  ...
  const numericPlace = isDecimal ? 0 : typeof place === 'string' ? parseFloat(place) : place;
  const valueRoundedToPlace = isDecimal ? 0 : getValueRoundedToPlace(value, numericPlace);
  // Initialize to 0 so the spring animates up to valueRoundedToPlace
  ...
  return (
    <span className="counter-digit" style={{ height, ...digitStyle }}>
      {Array.from({ length: 10 }, (_, i) => (
        <Number key={i} mv={animatedValue as unknown as MotionValue} number={i} height={height} />
      ))}
    </span>
  );
}
